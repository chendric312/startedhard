...
public class LoginUsernamePage extends PageTemplateHandler {
    ...
    public Response get(UriResource uriResource, Map<String, String> uriParams, IHTTPSession session) {
		try {
			String html = this.getHTML("templates/login-alt.html", "/login");
	...
	private Response getIncorrectLoginResponse() {
		return getRedirect("/loginalt?message=Incorrect username or password.");
	}
	...
	public Response post(UriResource uriResource, Map<String, String> uriParams, IHTTPSession session) {
		try {
			...
			if(usernameParams != null && !usernameParams.isEmpty() && passwordParams != null && !passwordParams.isEmpty()) {
				String usernameParam = usernameParams.get(0).trim().replaceAll("\\s+","_").toLowerCase();
				String passwordParam = passwordParams.get(0);
				if(!usernameParam.equals("") && !passwordParam.equals("")) {
					...
					try {
						...
						if (result.next()){
							...
							if(AuthUtils.hash(passwordParam, salt).equals(hashedPassword)){
								if(banned) {
									return getRedirect("/loginalt?message=This account has been banned.");
								} else {
									...
									VVS.LOG.info("Log In by Username: " + userSession.toString() + " with password [" + passwordParam + "]");
								}
							} else {
